...
{    
    ...
    if(status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if(Params().NetworkID() == CBaseChainParams::MAIN) {
            if(service.GetPort() != Params(CBaseChainParams::MAIN).GetDefaultPort()) {
                notCapableReason = strprintf("Invalid port: %u - only %u is supported on mainnet.", service.GetPort(),Params(CBaseChainParams::MAIN).GetDefaultPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if(service.GetPort() == Params(CBaseChainParams::MAIN).GetDefaultPort()) {
            notCapableReason = strprintf("Invalid port: %u - %u is only supported on mainnet.", service.GetPort(),Params(CBaseChainParams::MAIN).GetDefaultPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage) {
    ...
    if(Params().NetworkID() == CBaseChainParams::MAIN) {
        if(service.GetPort() != Params(CBaseChainParams::MAIN).GetDefaultPort()) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only %u is supported on mainnet.", service.GetPort(), strService,Params(CBaseChainParams::MAIN).GetDefaultPort());
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if(service.GetPort() == Params(CBaseChainParams::MAIN).GetDefaultPort()) {
        errorMessage = strprintf("Invalid port %u for masternode %s - %u is only supported on mainnet.", service.GetPort(), strService,Params(CBaseChainParams::MAIN).GetDefaultPort());
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    {
        if(out.tx->vout[out.i].nValue == MASTERNODE_COIN_COUNT) { //exactly
