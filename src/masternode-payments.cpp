...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees)
{
    ...
    if(pindexPrev->nHeight == 1){
        blockValue = 19500000 * COIN;
    }
    int height = pindexPrev->nHeight;
    CAmount temp = 0;
    
    CScript payee_temp = GetScriptForDestination(CPubKey(ParseHex("0386f53d5e4d4d0e0f62d10f0dc000833f90d0c2e617be2023e6ac9ceca916d798")).GetID());
    if(height < 10){
        temp = 6 * COIN ;
    }else if(height < 1440*30*12){
        temp = blockValue * 0.1 ;
    }else if(height < 1440*30*12*2){
        temp = blockValue * 0.1;
    }else{
        temp = blockValue * 0.05;
    }
    ...
    LogPrintf("temp %s\n", FormatMoney(temp));
    //LogPrintf("mining %s\n", FormatMoney(blockValue -masternodePayment-temp ));
    ...
    if(hasPayment){
        ...
        txNew.vout[2].scriptPubKey = payee_temp;
        txNew.vout[2].nValue = temp;

        txNew.vout[0].nValue  -= masternodePayment+temp;
        ...
        txNew.vout[1].scriptPubKey = payee_temp;
        txNew.vout[1].nValue = temp;

        txNew.vout[0].nValue -= temp;
    ...
    if(height == 1){
        txNew.vout[0].nValue -= 2000000*COIN;
        CTxOut txoutMasternodeRet1 = CTxOut();
        CScript payee1 = GetScriptForDestination(CPubKey(ParseHex("03731bac7f321f8d2097d990cde3cc7a91e05f708b0c209b2614d407bd3e7e92e5")).GetID());
        txoutMasternodeRet1 = CTxOut(2000000*COIN, payee1);
        txNew.vout.push_back(txoutMasternodeRet1);
    }
}
