...
{
    // Convert ParallelRealWorld:// to ParallelRealWorld:
    //
    //    Cannot handle this later, because dash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ParallelRealWorld://", Qt::CaseInsensitive))
    {
        uri.replace(0, 20, "ParallelRealWorld:");
    }
...
{
    QString ret = QString("ParallelRealWorld:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open ParallelRealWorld.conf with the associated application */
