...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    CAmount max = 6800000;
    int64_t temp = total/100000000;
    //LogPrintf("-----------------Verifying mncache.dat format max: %s...\n",max);
    //LogPrintf("-----------------Verifying mncache.dat format temp:%s...\n",temp);
    if( temp > max){
        return MAX_AMOUNT;
    }
...

    {
        ...
        /*
        if(recipients[0].useInstantX && total > 2000000*COIN){
            emit message(tr("Send Coins"), tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to %1 DASH.").arg(2000000),
