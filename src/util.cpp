...
boost::filesystem::path GetDefaultDataDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ParallelRealWorld";
#else
    fs::path pathRet;
    ...
    return pathRet / "ParallelRealWorld";
#else
    // Unix
    return pathRet / ".ParallelRealWorld";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PRW.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ParallelRealWorld.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ParallelRealWorld.conf
